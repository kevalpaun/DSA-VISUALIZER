...
export function getRandomArrayAscending(length = generateRandomNumberAscending(5, 30)) {
  return Array.from(new Array(length), () => generateRandomNumberAscending());
}
...
function generateRandomNumberAscending(lowerLimit = 0, upperLimit = 999) {
  return sort(lowerLimit + Math.floor(Math.random() * upperLimit));
}